    ...
    foreach ($value as $key => $value2) {
      if ($value2 != '' && $key != '0') {
        ...
        if (count($value[$key]) == 0)
          unset($value[$key]); # delete any empty arrays
      } else {
...
function updateOrderData()
{
  ...
  if (isset($orderdata['product']) && is_array($orderdata['product']))
  ...
  if ($_SESSION['introcode'] > '')
    $discamount = round($priceextax * $_SESSION['discount'], 2);
...
function prepareData()
{
  ...
  if (isset($orderData['guardian'][1]['fullname']) && preg_match('/and/i', $orderData['guardian'][1]['fullname']))
    $orderData['guardian'][1]['couple'] = 1;
  $orderData['guardian'][2]['couple'] = 0;
  if (isset($orderData['guardian'][2]['fullname']) && preg_match('/and/i', $orderData['guardian'][2]['fullname']))
    $orderData['guardian'][2]['couple'] = 1;

  # unset guardian 2 if $orderData['guardian'][2]['fullname'] empty
  if (!isset($orderData['guardian'][2]['fullname']))
    unset($orderData['guardian'][2]);
  ...
  # age of inheritance
  if (!isset($orderData['ageOfInheritance']))
    $orderData['ageOfInheritance'] = 18;

  # young children
  if (isset($orderData['hasChildrenYoung']) && $orderData['hasChildrenYoung'] == 'Yes')
    $hasChildrenYoung = true;
  else
    $hasChildrenYoung = false;
  ...
  foreach ($theArrays as $name => &$theArray) {
    if (is_array($theArray)) {
      for ($i = 1; $i <= count($theArray); $i++) {
        ...
        if ($theArray[$i]['altrecipient']) {
          ...
          if ($altrecipient == 'Divide') {
            ...
            for ($k = 1; $k < $childcount + 1; $k++) {
              if ($k == $childNum)
                continue; # if the primary beneficiary is one of the children, then we need to exclude that one from the alternatebeneficiary
        ...
        if ($name == 'devise')
          $theArray[$i]['property'] = replace_ws($theArray[$i]['property']);
        ...
        if (!isset($theArray[$i]['condition'])) {
          ...
          if (preg_match('/\.$/', $theArray[$i]['condition']) == 0)
            $theArray[$i]['condition'] .= '.';
          $theArray[$i]['condition'] .= ' ';
        }
      }
...
function prepareQuickWillData()
{
  ...
  if (isset($qwOrderData['guardianFullName']) && preg_match('/and/i', $qwOrderData['guardianFullName']))
    $qwOrderData['guardianCouple'] = 1;

  # young children
  if (isset($qwOrderData['hasChildrenYoung']) && $qwOrderData['hasChildrenYoung'] == 'Yes')
    $hasChildrenYoung = true;
  else
    $hasChildrenYoung = false;
  ...
  if (!isset($qwOrderData['specificGifts'][0]) || $qwOrderData['specificGifts'][0] == '')
    unset($qwOrderData['specificGifts']);
  ...
  if (!isset($qwOrderData['requirements'][0]) || $qwOrderData['requirements'][0] == '')
    unset($qwOrderData['requirements']);
...
function get_linked_users($introcode, $level, &$userlevel, &$dbh)
{
  ...
  if (isset($row)) {
    ...
    if ($level > 1 && $row['introcode'] > '')
      get_linked_users($row['introcode'], $level - 1, $userlevel, $dbh);
  }
}
...
function initiateCBASession()
{
  $sessionIdURL = "https://paymentgateway.commbank.com.au/api/rest/version/100/merchant/" . CBA_MERCHANTID . "/session";
  # call API to get session ID, there is no body to post and the response includes session.id
  $curl = curl_init();
  curl_setopt_array($curl, array(
    CURLOPT_URL => $sessionIdURL,
    ...
    CURLOPT_HTTPHEADER => array(
      ...
      'Authorization: Basic ' . base64_encode('merchant.' . CBA_MERCHANTID . ':' . CBA_APIPW)
    )
  ));
  ...
  if (isset($responseData['session']['id'])) {
    ...
    error_log('CBA get session ID success: ' . $_SESSION['sessionId']);
    $_SESSION['sessionId'] = $responseData['session']['id'];
    $_SESSION['aes256Key'] = $responseData['session']['aes256Key'];
  } else {
    error_log('CBA get session ID error: invalid response ' . $response);
  }
}

function getCBASessionData($sessionId)
{
  $sessionURL = "https://paymentgateway.commbank.com.au/api/rest/version/100/merchant/" . CBA_MERCHANTID . "/session/" . $sessionId;
  ...
  if ($httpCode == 200 && isset($responseData['session'])) {
    error_log('CBA session data retrieved: ' . json_encode($responseData));
    return $responseData;
  } else {
    error_log('CBA get session data error: HTTP ' . $httpCode . ' - ' . $response);
    return null;
  }
}

function doCBAPayment($orderId, $transactionId, $sessionId)
{
  $paymentURL = "https://paymentgateway.commbank.com.au/api/rest/version/100/merchant/" . CBA_MERCHANTID . "/order/" . $orderId . "/transaction/" . $transactionId;
  ...
  if ($httpCode == 200 || $httpCode == 201) {
    error_log('CBA payment success: ' . json_encode($responseData));
    return $responseData;
  } else {
    error_log('CBA payment error: HTTP ' . $httpCode . ' - ' . $response);
    return $responseData; // Return even on error so caller can handle it
  }
}

function updateCBASession($sessionId, $amount, $currency = 'AUD')
{
  $sessionURL = "https://paymentgateway.commbank.com.au/api/rest/version/100/merchant/" . CBA_MERCHANTID . "/session/" . $sessionId;
  ...
  if ($httpCode == 200 || $httpCode == 201) {
    // error_log('CBA session update success: ' . json_encode($responseData));
    return $responseData;
  } else {
    error_log('CBA session update error: HTTP ' . $httpCode . ' - ' . $response);
    return $responseData; // Return even on error so caller can handle it
  }
}
